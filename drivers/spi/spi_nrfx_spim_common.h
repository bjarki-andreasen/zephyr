...
struct spi_nrfx_common_data {
	...
	spi_nrfx_data_common_wake_handler wake_handler;
#endif
};
...
#define SPI_NRFX_COMMON_CS_GPIO_INIT(idx, inst) \
	GPIO_DT_SPEC_INST_GET_BY_IDX(inst, cs_gpios, idx)

#define SPI_NRFX_COMMON_CS_GPIOS_DEFINE(inst)							\
	static const struct gpio_dt_spec CONCAT(cs_gpios, inst)[] = {				\
		LISTIFY(									\
			DT_INST_PROP_LEN_OR(inst, cs_gpios, 0),					\
			SPI_NRFX_COMMON_CS_GPIO_INIT,						\
			(,),									\
			inst									\
		)										\
	}
...
#define SPI_NRFX_COMMON_DATA_WAKE_INIT(inst)							\
	IF_ENABLED(										\
		SPI_NRFX_HAS_WAKE,								\
		(										\
...
#define SPI_NRFX_COMMON_DATA_INIT(inst)								\
	{											\
		...
		SPI_NRFX_COMMON_DATA_WAKE_INIT(inst)						\
	}
...
#define SPI_NRFX_COMMON_CONFIG_WAKE_INIT(inst)							\
	IF_ENABLED(										\
		SPI_NRFX_HAS_WAKE,								\
		(										\
...
#define SPI_NRFX_COMMON_CONFIG_INIT(inst, _evt_handler)						\
	{											\
		...
		.cs_gpios = CONCAT(cs_gpios, inst),						\
		.cs_gpios_size = ARRAY_SIZE(CONCAT(cs_gpios, inst)),				\
		.pcfg = PINCTRL_DT_INST_DEV_CONFIG_GET(inst),					\
		.max_freq = DT_INST_PROP(inst, max_frequency),					\
		.max_transfer_len = BIT_MASK(DT_INST_PROP(inst, easydma_maxcnt_bits)),		\
		.orc = DT_INST_PROP(inst, overrun_character),					\
		SPI_NRFX_COMMON_CONFIG_INIT_RX_DELAY(inst)					\
		SPI_NRFX_COMMON_CONFIG_WAKE_INIT(inst)						\
